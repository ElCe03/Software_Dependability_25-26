...
public class FrontController {
    ...
    private void loadInterfaceScene(ActionEvent event) {
        try {
            ...
            Parent root = FXMLLoader.load(getClass().getResource("/services_list.fxml"));
    ...
    @FXML
    private void handleGetMedicineButton(ActionEvent event) {
        try {
            // Charge le fichier FXML de la liste des médicaments (par exemple : medicine_list.fxml)
            Parent medicineRoot = FXMLLoader.load(getClass().getResource("/catalogue.fxml"));

            // Récupère la fenêtre courante
            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();

            // Définit la nouvelle scène
            Scene scene = new Scene(medicineRoot);
            stage.setScene(scene);
            stage.setTitle("Available Medicines");
            stage.show();

        } catch (IOException e) {
            showErrorAlert("Failed to load medicine page", "Please try again later.");
            e.printStackTrace();
        }
    }
    ...
    public void handleDOSSIERMEDICAL(ActionEvent event) {
        try {
            // Load patient_dashboard.fxml from resources
    ...
    @FXML
    private void handleInfrastructureButton(ActionEvent event) {
        try {
            // Load listdepartement.fxml from resources
            Parent root = FXMLLoader.load(getClass().getResource("/listedepartement.fxml"));
            ...
            // Update title
            currentStage.setTitle("Department List");

        } catch (IOException e) {
            showErrorAlert("Navigation Error",
                    "Could not load department list",
                    e.getMessage());
        }
    }
